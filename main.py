...
class ServiceManager:
    ...
    def enable_ftp_feature(self):
        ...
        self.run_command("dism /online /enable-feature /featurename:IIS-WebServerRole /all")
        self.run_command("dism /online /enable-feature /featurename:IIS-FTPServer /all")
        self.log("IIS和FTP服务功能已启用")

    def start_ftp(self):
        self.log("正在启动FTP服务器...")
        self.run_command("net start ftpsvc")
        self.log("FTP服务器已启动")
        messagebox.showinfo("信息", "FTP服务器已启动")

    def stop_ftp(self):
        self.log("正在停止FTP服务器...")
        self.run_command("net stop ftpsvc")
        self.log("FTP服务器已停止")
        messagebox.showinfo("信息", "FTP服务器已停止")

    def start_iis(self):
        self.log("正在启动IIS...")
        self.run_command("start inetmgr")
        self.log("IIS已启动")
    ...
    def start_smb(self):
        ...
        self.run_command("dism /online /enable-feature /featurename:SMB1Protocol /all")
        self.log("SMB 服务已启动")
        messagebox.showinfo("信息", "SMB服务已经启用（可能需要重启）")

    def stop_smb(self):
        self.log("正在停止SMB服务器...")
        self.run_command("dism /online /disable-feature /featurename:SMB1Protocol")
        self.log("SMB服务器已停止")
    ...
    def start_nfs(self):
        ...
        self.run_command("dism /online /enable-feature /featurename:ServicesForNFS-Server /all")
        self.log("NFS 服务已启用")
        messagebox.showinfo("信息", "NFS 服务已启用")

    def stop_nfs(self):
        self.log("正在停止 NFS 服务...")
        self.run_command("dism /online /disable-feature /featurename:ServicesForNFS-Server")
        self.log("NFS 服务器已停止")
